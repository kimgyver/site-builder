...
- 외부 스케줄러(QStash 권장)에서 매 1분 호출
- 호출 대상: `GET /api/cron/publish`
- 인증 헤더: `Authorization: Bearer <CRON_SECRET>`
- 실제 발행 실행은 Settings의 `Publish scheduler interval (minutes)` 값(기본 5분)으로 제어
...
Upstash QStash에서 아래와 같이 스케줄을 생성한다.

- Destination URL: `https://<your-domain>/api/cron/publish`
- Cron: `* * * * *`
...
- 관리자: `/admin/settings`
- 항목: `Publish scheduler interval (minutes)`
- 기본값: `5`
- 허용 범위: `1` ~ `60`
...
- [ ] QStash 스케줄이 `/api/cron/publish`를 1분 주기로 호출하도록 설정됨
- [ ] `/admin/settings`의 publish interval 값 확인
...
3. QStash가 실제 호출했는지(Vercel logs + QStash 로그) 확인
