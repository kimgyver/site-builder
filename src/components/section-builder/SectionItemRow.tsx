...
export function SectionItemRow({
  ...
  return (
    <div
      ...
      tabIndex={0}
      className={`flex w-full items-start justify-between gap-2.5 rounded-md border border-zinc-200 bg-white px-2.5 py-2 text-xs md:gap-3 md:px-3 md:py-2.5 md:text-sm ${draggedIndex === sectionIndex ? "opacity-50" : ""}`}
      draggable
      onDragStart={event => {
    ...
    >
      <div className="min-w-0 flex-1 space-y-1">
        <div className="flex flex-wrap items-center gap-2">
          <span className="inline-flex items-center rounded-full bg-zinc-100 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wide text-zinc-700 md:text-xs">
            {SECTION_CATALOG[section.type]?.label ?? section.type}
          </span>
          {!section.enabled && (
            <span className="inline-flex items-center rounded-full bg-amber-100 px-2 py-0.5 text-[10px] uppercase tracking-wide text-amber-700 md:text-xs">
              hidden (public)
            </span>
          )}
          <span
      ...
      <div className="flex flex-col items-end gap-1 text-[10px] md:text-xs">
        ...
        <button
          type="button"
          className="rounded border border-red-200 px-1.5 py-0.5 text-red-600 hover:border-red-300 hover:bg-red-50 md:px-2 md:py-1"
